  ...
  if (!route) {
    return (
      <div className={`p-6 rounded-2xl border text-center space-y-3 ${
        ...
        <div className="w-10 h-10 rounded-xl bg-blue-600/20 text-blue-500 flex items-center justify-center mx-auto border border-blue-500/30">
          <Navigation className="w-5 h-5" />
        </div>
        <h4 className={`text-sm font-extrabold ${darkMode ? 'text-white' : 'text-slate-900'}`}>No Active Safe Route</h4>
    ...
    <div className={`p-4 sm:p-5 rounded-2xl border transition-all text-left space-y-4 ${
      ...
      <div className={`flex items-center justify-between gap-2 flex-wrap border-b pb-3 ${
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="flex items-center gap-1.5">
              <span className={`text-xs font-black uppercase tracking-wider ${darkMode ? 'text-white' : 'text-slate-900'}`}>
                Adaptive SafeRoute AI
              </span>
              <span className="px-1.5 py-0.2 rounded text-[9px] font-bold bg-blue-900/50 text-blue-300 dark:text-blue-300 border border-blue-700/50">
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className={`px-2.5 py-1 rounded-lg text-[11px] font-bold flex items-center gap-1 border cursor-pointer transition-colors ${
              darkMode 
      ...
      {disruptionMessage && (
        <div className={`p-3 rounded-xl border flex items-start gap-2.5 animate-pulse ${
          isRecalculating 
            ? 'bg-blue-950/40 border-blue-800/60 text-blue-200 dark:text-blue-200' 
            : 'bg-amber-950/40 border-amber-800/60 text-amber-200 dark:text-amber-200'
        }`}>
      ...
      {simulationMenuOpen && (
        <div className={`p-3 rounded-xl border space-y-2 text-xs ${
          darkMode ? 'bg-slate-950/90 border-slate-700/80' : 'bg-slate-50 border-slate-200'
        }`}>
          <div className={`font-bold flex items-center justify-between ${darkMode ? 'text-slate-300' : 'text-slate-800'}`}>
            <span>Select Route Disruption Simulation:</span>
            <span className={`text-[10px] ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>Active: {activeScenario}</span>
          </div>
      ...
      <div className={`grid grid-cols-3 gap-2 p-2.5 rounded-xl border text-center ${
        ...
        <div>
          <div className={`text-[10px] uppercase font-bold ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>Total Distance</div>
          <div className={`text-base font-black mt-0.5 ${darkMode ? 'text-white' : 'text-slate-900'}`}>{route.totalDistanceKm} km</div>
        </div>
        <div>
          <div className={`text-[10px] uppercase font-bold ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>Travel Time</div>
          <div className="text-base font-black text-blue-600 dark:text-blue-400 mt-0.5">{route.totalMinutes} min</div>
        </div>
        <div>
          <div className={`text-[10px] uppercase font-bold ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>Corridor Risk</div>
          <div className="text-base font-black text-emerald-600 dark:text-emerald-400 mt-0.5">{route.overallRisk}</div>
        </div>
      </div>
      ...
      <div className={`p-3 rounded-xl border space-y-2 ${
        ...
        <div className="flex items-center justify-between text-xs">
          <span className={`font-medium ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>Destination Safe Zone:</span>
          <span className={`font-bold flex items-center gap-1 ${darkMode ? 'text-white' : 'text-slate-900'}`}>
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-500" />
            {route.destinationName}
          </span>
        </div>
        <div className={`flex items-center justify-between text-[11px] ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>
          <span>Capacity Check: <b>{route.safeZoneAvailable}</b> beds available / {route.safeZoneCapacity} total</span>
          <span className={route.capacityStatus === 'CONSTRAINED' ? 'text-rose-500 font-bold' : 'text-emerald-600 dark:text-emerald-400 font-bold'}>
      ...
      <div className="space-y-2">
        <div className={`text-xs font-extrabold uppercase tracking-wide flex items-center justify-between ${
          darkMode ? 'text-slate-300' : 'text-slate-700'
        }`}>
          <span>Multimodal Transfer Steps</span>
          <span className={`text-[10px] font-normal ${darkMode ? 'text-slate-500' : 'text-slate-400'}`}>Click segment to focus map</span>
        </div>

        <div className={`space-y-2 divide-y ${darkMode ? 'divide-slate-800/50' : 'divide-slate-200'}`}>
          {route.segments.map((seg, idx) => (
            <div
              key={seg.id}
              onClick={() => onFocusSegmentOnMap && onFocusSegmentOnMap(seg)}
              className={`pt-2 flex items-start gap-3 p-1.5 rounded-lg transition-colors cursor-pointer group ${
                darkMode ? 'hover:bg-slate-800/40' : 'hover:bg-slate-100'
              }`}
            >
              <div className={`w-8 h-8 rounded-lg border flex items-center justify-center shrink-0 mt-0.5 group-hover:scale-105 transition-transform ${
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-1">
                  ...
                  <div className={`text-xs font-bold shrink-0 ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                    {seg.distanceKm} km &bull; {seg.durationMinutes} min
                  </div>
                </div>

                <div className={`text-[11px] mt-0.5 flex items-center gap-2 ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>
              ...
              <ChevronRight className={`w-4 h-4 shrink-0 mt-2 ${
                darkMode ? 'text-slate-500 group-hover:text-white' : 'text-slate-400 group-hover:text-slate-800'
              }`} />
            </div>
          ))}
        </div>
